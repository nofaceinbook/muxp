...
def earclip(polygon):
    ...
    if is_clockwise(polygon):  #### WAS PRIVATE STARTING WITH _
        polygon.reverse()
    ...
    for i in range(point_count):
        ...
        #print("Checking ear at: {}".format(point))  ##### TESTING ONLY ####

        if _is_ear(prev_point, point, next_point, polygon):
            ear_vertex.append(point)
            #print("Ear found at: ".format(point))  ##### TESTING ONLY ####
    ...
    while ear_vertex and point_count >= 3:
        ...
        polygon.remove(ear)
        point_count -= 1
        triangles.append(((next_point.x, next_point.y), (ear.x, ear.y), (prev_point.x, prev_point.y))) ### swapped first and third vertex of tria in order to get clockwise ordered trias!!
        if point_count > 3:
...
def is_clockwise(polygon):  #### WAS PRIVATE  STARTING WITH _
    s = 0
    polygon_count = len(polygon)
    for i in range(polygon_count):
        point = polygon[i]
        point2 = polygon[(i + 1) % polygon_count]
        s += (point2[0] - point[0]) * (point2[1] + point[1])  ### WAS .x  .y
    return s > 0
...
def _is_ear(p1, p2, p3, polygon):
    ...
    #print("   Contains no points: {}".format(_contains_no_points(p1, p2, p3, polygon)))   ###### TESTING ONLY ####
    #print("   Contains no points: {}".format(_is_convex(p1, p2, p3)))   ###### TESTING ONLY ####
    #print("   Triangle aerea:     {}".format(_triangle_area(p1.x, p1.y, p2.x, p2.y, p3.x, p3.y))) ###### TESTING ONLY ####
    return ear
...
def _is_point_inside(p, a, b, c):
    ...
    #print ("            AreaDiff: {} < {} ?".format(abs(area - sum([area1, area2, area3])), EPSILON))
